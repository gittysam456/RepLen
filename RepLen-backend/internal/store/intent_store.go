...
if i.Status == intent.StatusPending && (i.ExecuteAt.Before(time.Now()) || i.ExecuteAt.Equal(time.Now())) {
pending++
}
if i.Status == intent.StatusExecuted {
executed++
}
}
...
for _, i := range s.intents {
if i.Status == intent.StatusPending && (i.ExecuteAt.Before(now) || i.ExecuteAt.Equal(now)) {
